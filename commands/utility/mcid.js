    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === 'add') {
            ...
            const createFile = (pathName, source) => {
                ...
                createFile(`C:/Users/user/Desktop/DiscordBots/MedaTrainBot_JS-main/MCIDdata/${addmcid}.json`, testObj);
            }
        } else if (interaction.options.getSubcommand() === 'list') {
            const files = fs.readdirSync('C:/Users/user/Desktop/DiscordBots/MedaTrainBot_JS-main/MCIDdata');
            interaction.reply({ content: `All MCID Files:${files}` })
            /*
        } else if (interaction.options.getSubcommand() === 'delete') {
                var deletemcid = interaction.options.getString("deletemcid")
                const url = `C:/Users/user/Desktop/DiscordBots/MedaTrainBot_JS-main/MCIDdata/${deletemcid}.json`;
                fetch(url)
